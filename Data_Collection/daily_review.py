...
#reg = pd.read_csv("C:/Users/Lenovo/Documents/카카오톡 받은 파일/야구/Regular_Season_Batter_Day_by_Day_b4.csv",encoding='cp949')
#reg.iloc[0,:]
#reg.columns
...
for j in range(2017,2019):
    ...
    for k in range(3,5):
        ...
        for i in range(1,len(soup.select('tr'))):
            ...
            if soup.select('tr')[i].select('td')[-1].get_text() == '-':
                
                link = soup.select('tr')[i].select('.relay')[0].find("a")["href"]
                driver.get('https://www.koreabaseball.com' + link)
            else:
                pass
...
#  soup.select('tr')[5].select('td')[-1].get_text()          
#            
#            
#            reviewss = soup.select('tr')[i].select('.relay')[0].get_text()
#            if reviewss == '리뷰':
#                link = soup.select('tr')[i].select('.relay')[0].find("a")["href"]
#                driver.get('https://www.koreabaseball.com' + link)
#            elif reviewss =='':
